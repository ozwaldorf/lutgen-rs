...
/// Constrains a transformation to keep content within view bounds.
///
/// When content is smaller than the view, centers it.
/// When content is larger than the view, constrains panning to edges.
fn constrain_transform_to_content(
    ...
    if content_size.x <= view_size.x {
        ...
        // Content is larger than view, constrain to edges
...
/// * Supports zooming
/// * Has no scroll bars
/// * Has constrained panning based on content bounds
#[derive(Clone, Debug)]
...
impl Scene {
    ...
    /// Enable or disable panning constraints.
    ///
    /// When enabled (default), panning is constrained to the content bounds:
    /// - Content smaller than view is centered
    /// - Content larger than view is constrained to edges
    #[inline]
    ...
    pub fn show<R>(
        ...
        if !scene_rect_was_good {
            ...
            // Recalculates transform based on inner_rect, resulting in a rect that's the full size
            // of outer_rect but centered on inner_rect.
    ...
    fn show_global_transform<R>(
        ...
        // Update content rect for constraint calculations
